...
async def surface_search_patched(
    ...
    if exact_bucket:
        ...
        if (
            is_archived
            and archived_original_kind not in search_excluded_types()
        ...
        _exact_type = meta.get("type")
        _exact_feel_ok = _exact_type == "feel" and "feel" not in search_excluded_types()
        if (
            not is_archived
            and _exact_type not in search_excluded_types()
            and (_exact_feel_ok or _can_surface_search(exact_bucket))
            and _bucket_has_tags(meta, tag_filter)
            and _bucket_in_created_range(exact_bucket, created_from, created_to)
        ):
            if meta.get("type") == "feel":
                header = feel_hit_header(exact_bucket["id"], meta)
            else:
                protected_mark = (
                    "🛡️ [受保护记忆] "
                ...
                header = f"{protected_mark}[exact_bucket_id:true] [bucket_id:{exact_bucket['id']}]"
    ...
    search_kwargs = {
        "limit": max(max_results, _VECTOR_QUERY_TOPK),
    ...
    for bucket in matches:
        ...
        if _is_archived(bucket):
            ...
            if original_kind in search_excluded_types():
                continue
        else:
    ...
    for bucket in matches:
        ...
        if _is_archived(bucket):
            ...
            rendered, entry_tokens = render_stored_bucket(
                bucket, feel_hit_header(bucket_id, meta), _footprint(bucket)
            )
